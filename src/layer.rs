...
use crate::ohos::{OHOSWriter, CappedTag};

/// A `Layer` that logs events and spans to the OpenHarmony's HiLog.
/// It implements the `tracing_subscriber`'s [`Layer`] trait.
///
...
impl Layer {
    /// Creates a new `Layer` with the given `domain` and `tag`.
    pub fn new(domain: u16, tag: &str) -> io::Result<Self> {
        let tag = CappedTag::new(tag.as_bytes())?;
        Ok(Self { domain, tag })
    }
}
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    fn on_event(&self, event: &Event, ctx: Context<S>) {
        let mut writer = OHOSWriter::new(event.metadata().level(), self.domain, &self.tag); //PlatformLogWriter::new(event.metadata().level(), &self.tag);
